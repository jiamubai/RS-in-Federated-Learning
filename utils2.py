...
from model import *
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, CIFAR10_pretrain_truncated, SVHN_custom, \
    FashionMNIST_truncated, CIFAR100_pretrain_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData
...
def load_cifar10_data_pre(datadir):
    transform = transforms.Compose([transforms.ToTensor()])

    cifar10_train_ds = CIFAR10_pretrain_truncated(datadir, train=True, download=True, transform=transform)
    cifar10_test_ds = CIFAR10_pretrain_truncated(datadir, train=False, download=True, transform=transform)
...
def load_cifar100_data_pre(datadir):
    transform = transforms.Compose([transforms.ToTensor()])

    cifar100_train_ds = CIFAR100_pretrain_truncated(datadir, train=True, download=True, transform=transform)
    cifar100_test_ds = CIFAR100_pretrain_truncated(datadir, train=False, download=True, transform=transform)
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    elif dataset == 'cifar10_pre':
        X_train, y_train, X_test, y_test = load_cifar10_data_pre(datadir)
    elif dataset == 'cifar100_pre':
        X_train, y_train, X_test, y_test = load_cifar100_data_pre(datadir)
    ...
    if partition == "homo":
    ...
    elif partition > "noniid-#label0" and partition <= "noniid-#label99":
        print(partition)
        ...
        if dataset in ('celeba', 'covtype', 'a9a', 'rcv1', 'SUSY'):
            ...
            K = 10
        if num == 10 and dataset!='cifar100':
            ...
            occ = [i for i in range(K)]
            while len(occ) != n_parties * num:
                if len(occ) < n_parties * num:
                    # ind = random.randint(0, K - 1)
                    ind = len(occ) - K * (len(occ) // K)
                    occ.append(ind)
                else:
                    ind = random.randint(0, len(occ))
                    occ.pop(ind)
            times = [occ.count(i) for i in range(K)]
            occ = np.array(occ)
            np.random.shuffle(occ)
            contain = occ.reshape((n_parties, num))
            net_dataidx_map = {i: np.ndarray(0, dtype=np.int64) for i in range(n_parties)}
            print(times)
            print(contain)
            for i in range(K):
                ...
                for j in range(n_parties):
                    if i in contain[j]:
                        ...
                        net_dataidx_map[j] = np.append(net_dataidx_map[j], idx_k)
                        ids += 1
...
def compute_accuracy(model, dataloader, args_model, args_pretrain, model_pre, get_confusion_matrix=False, device="cpu"):
    if model_pre is None:
        print('No pretrain model selected')
    ...
    with torch.no_grad():
        if model_pre == 'pre':
            for batch_idx, tmp in enumerate(dataloader):
            ...
            for tmp in dataloader:
                for batch_idx, (x, target) in enumerate(tmp):
                    x, target = x.to(device), target.to(device, dtype=torch.int64)
                    # if (args_model == 'RS' or args_model == 'mlp') and args_pretrain != 'yes':
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0):
    if dataset in (
    'mnist', 'femnist', 'fmnist', 'cifar10', 'cifar100','svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar10_pre', 'cifar100_pre'):
        if dataset == 'mnist':
            ...
            # print('in')
            dl_obj = CIFAR10_pretrain_truncated
            ...
            # print('in')
            dl_obj = CIFAR100_pretrain_truncated
        ...
        # print('test_dl.shape:', test_dl.shape)
