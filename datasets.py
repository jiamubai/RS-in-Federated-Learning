...
import numpy as np
from torchvision.datasets import MNIST, CIFAR10, SVHN, FashionMNIST, CIFAR100
...
class CIFAR10_pretrain_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        if self.dataidxs is not None:
            data = data[self.dataidxs]
            target = target[self.dataidxs]
            # print('self.dataidxs:', len(self.dataidxs))
            batch_size = 100
            iter_num = len(self.dataidxs) // 100 + 1
            total = len(self.dataidxs)
        else:
            iter_num = 500
            batch_size = 100
            total = 50000
        with torch.no_grad():
            ...
            model_pre = resnet50(weights=weights)
            model_pre = model_pre.to('cuda:7')
            # data, target = torch.from_numpy(data).to('cuda:0'), torch.from_numpy(target).to('cuda:0')
            # print(data.shape)
            ...
            for i in range(iter_num):
                ...
                data_batch = torch.from_numpy(data_batch).permute(0, 3, 1, 2).to('cuda:7')
                # print(data_batch.shape)
                data_batch = preprocess(data_batch)
                # print(data_batch.shape)
                if i == 0 :
                    output = model_pre(data_batch)
                else:
                    output = torch.cat((output, model_pre(data_batch)), dim=0)
        output = torch.reshape(output, (len(target), 100, 10))
        output = output.cpu().numpy()
        # print(output.shape)
        # print('cifar10 pretrained ')
...
class CIFAR100_pretrain_truncated(data.Dataset):
        ...
        with torch.no_grad():
            ...
            for i in range(iter_num):
                if (i+1)*batch_size <= total:
                    ...
                    data_batch = data[i * batch_size:]
                data_batch = torch.from_numpy(data_batch).permute(0,3,1,2).to('cuda:6')
                # print(data_batch.shape)
                data_batch = preprocess(data_batch)
                # print(data_batch.shape)
                if i == 0 :
